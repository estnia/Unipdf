...
# 复制 wheel 文件
echo "复制依赖包..."
if ls "${PROJECT_DIR}/wheels"/*.whl 1>/dev/null 2>&1; then
    cp "${PROJECT_DIR}/wheels"/*.whl "${BUILD_DIR}/usr/lib/unipdf-deps/wheels/"
else
    echo "警告: 未找到 wheel 文件"
    exit 1
fi
...
# 移动结果到项目根目录
mv "${PACKAGE_NAME}_${VERSION}_${ARCH}.deb" "${PROJECT_DIR}/"
...
echo ""
echo "=== 构建完成 ==="
echo "输出: ${PROJECT_DIR}/${PACKAGE_NAME}_${VERSION}_${ARCH}.deb"
echo ""
echo "安装方法:"
