...
# 复制源代码到 /opt/unipdf/
echo "复制源代码..."
cp "${PROJECT_DIR}/main.py" "${BUILD_DIR}/opt/${PACKAGE_NAME}/"
cp -r "${PROJECT_DIR}/pdfviewer" "${BUILD_DIR}/opt/${PACKAGE_NAME}/"
...
if ls "${WHEELS_DIR}"/*.whl 1>/dev/null 2>&1; then
    cp "${WHEELS_DIR}"/*.whl "${BUILD_DIR}/opt/${PACKAGE_NAME}/wheels/"
else
